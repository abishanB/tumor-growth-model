...
double Exposure(double t) {
  //hills equation
  return 30.0 * pow(t, 0.5) / (pow(10.0, 0.5) + pow(t, 0.5));
}
...
int main() {
  ...
  plt::plot(time_values, S_values, {{"label", "S (viable cells)"}, { "color", "green"}});
  plt::plot(time_values, D_values, {{"label", "D (damaged cells)"}, { "color", "red"}});
  plt::plot(time_values, T_values, {{"label", "T (total tumor size, T = S + D)"}, { "color", "blue"}});
